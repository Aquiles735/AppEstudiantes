...
class Control:
    ...
    def __init__(self, window):
        self.wind = window
        self.wind.title('Sistema de Registro de Estudiantes')
        self.wind.geometry('1000x700') # Define un tamaño inicial para la ventana
        self.wind.config(bg='#2c3e50') # Color de fondo principal
        ...
        # --- Widgets de la ventana principal ---
        frame = LabelFrame(self.wind, text='Registro de Estudiantes', bg='#34495e', fg='white', font=('Helvetica', 12, 'bold'))
        frame.grid(row=0, column=0, columnspan=3, pady=20, padx=100, sticky="ew")
        ...
        btn_descargar.grid(row=7, column=0, columnspan=2, sticky='we', padx=5, pady=5)
        btn_descargar.bind('<Return>', lambda event: btn_descargar.invoke())
        
        btn_salir = ttk.Button(frame, text='Salir', command=self.wind.destroy)
        btn_salir.grid(row=7, column=2, columnspan=2, sticky='we', padx=5, pady=5)
        btn_salir.bind('<Return>', lambda event: btn_salir.invoke())
        
        self.messaje = Label(frame, text='', bg='#34495e', fg='#e74c3c', font=('Helvetica', 10, 'bold')) # Rojo oscuro para errores
        self.messaje.grid(row=8, column=0, columnspan=4, sticky='we')

        # Treeview principal
        self.tree=ttk.Treeview(self.wind, columns=('col0', 'col1', 'col2', 'col3','col4','col5'), show='headings')
        self.tree.grid(row=1, column=0, columnspan=3, padx=10, pady=10, sticky="nsew") # Ajusta la posición del Treeview
        ...
        self.wind.grid_columnconfigure(2, weight=1)
