...
class VentanaNotas(tk.Toplevel):
    ...
    def guardar_nota(self):
        ...
        try:
            ...
            # Quitar la validación del nivel/sección si la cédula es de solo lectura,
            # ya que se asume que fue seleccionada del Treeview filtrado.
            # Si se desea mantener, se puede refactorizar para que la validación
            # se haga antes de cargar la nota, en la función `seleccionar_estudiante`.
...
class VentanaNotas:
    ...
    def __init__(self, master, cedula_estudiante=None, nivel_estudiante=None, seccion_estudiante=None, callback_actualizar=None):
        ...
        self.registrar_notas.transient(master)  # Hace que esta ventana sea modal
        self.registrar_notas.grab_set()
        self.registrar_notas.resizable(False, False)
        self.registrar_notas.config(bg='#34495e') # Color de fondo de la ventana
        ...
        if cedula_estudiante:
            self.cedula_entry.insert(0, cedula_estudiante)
            self.cedula_entry.config(state='readonly') # Se hace de solo lectura para evitar cambios accidentales
        ...
        btn_salir = ttk.Button(contenedor_widgets, text='Salir', command=self.registrar_notas.destroy, style='Accent.TButton')
        btn_salir.grid(row=5, column=1, pady=5, padx=5, sticky="we")
        ...
        self.tree.grid(row=0, column=1, rowspan=2, padx=10, pady=10, sticky="nsew") # Ajusta la posición del treeview
        
        # Scrollbar para el Treeview
        scrollbar = ttk.Scrollbar(self.registrar_notas, orient="vertical", command=self.tree.yview)
        scrollbar.grid(row=0, column=2, rowspan=2, sticky='ns')
        self.tree.configure(yscrollcommand=scrollbar.set)
        
        # Ajustes de columnas para que el treeview se expanda
        self.registrar_notas.grid_columnconfigure(1, weight=1)
        self.registrar_notas.grid_rowconfigure(1, weight=1)
        ...
        self.registrar_notas.update_idletasks()
        width = self.registrar_notas.winfo_width()
        height = self.registrar_notas.winfo_height()
        x = (self.registrar_notas.winfo_screenwidth() // 2) - (width // 2)
        y = (self.registrar_notas.winfo_screenheight() // 2) - (height // 2)
        self.registrar_notas.geometry(f'{width}x{height}+{x}+{y}')
